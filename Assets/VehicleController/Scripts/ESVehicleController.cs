...
public class ESVehicleController : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        m_startangdrag = CarRb.angularDamping;
    ...
    public void AddTorqueToEngine(float Accel, float Shoebrake)
    {
            ...
            case EngineSettings.CarDriveType.FourWheel:
                {
                    ...
                    for (int i = 0; i < m_wheelsettings.frontwheels.frontwheelcols.Length; i++)
                    {
                        ...
                        //
                        if (CurrentSpeed > 5 && Vector3.Angle(transform.forward, CarRb.linearVelocity) < 50f)
                        {
        ...
        m_wheelsettings.rearwheels.rearwheelcols[1].attachedRigidbody.AddForce(-transform.up * GripForce * CarRb.linearVelocity.magnitude);
        if (Drag > 0)
        {
            if (Accel == 0)
            {
                CarRb.linearDamping = Drag;
            }
            else
            {
                CarRb.linearDamping = 0.01f;
            }
        }
        CarRb.angularDamping = m_isgrounded == false ? AirDrag : m_startangdrag;
    ...
    public void BrakeSystem(float HandBrake)
    {
        ...
        {
            if (HandBrake > 0 && CarRb.linearVelocity.magnitude > 0.2f)
            {
                if (m_enginesettings.AIrBrakeTransform != null)
                {
    ...
    public void CarSpeed()
    {
        ...
        else
        {
            if (CurrentSpeed > m_gearshift.NitroTopSpeed)
                CarRb.linearVelocity = (m_gearshift.NitroTopSpeed / Pi) * CarRb.linearVelocity.normalized;
        }
    }
    //gimbal lock
    public void SteerBalance()
    {
        ...
        if (Shoebrake == 0)
        {
            if (!m_isDrifting)
            {
                ...
                {
                    SteerBalanceFactor = 0.61f;
                }
            }
            else
            {
                SteerBalanceFactor = .15f;
            }
        }
        else
        {
            SteerBalanceFactor = 1;
        }
        */
        for (int i = 0; i < m_wheelsettings.frontwheels.frontwheelcols.Length; i++)
        {
