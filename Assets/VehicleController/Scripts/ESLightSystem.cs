...
public class ESLightSystem : MonoBehaviour
{
    ...
    public void KeyInput()
    {
        ...
        if (gearshift.AutoReverse)
        {
            ...
            {
                if (gearshift.GetComponent<Rigidbody>().linearVelocity.magnitude < 5f && Input.GetAxis("Vertical") < 0)
                {
            ...
            {
                if (gearshift.GetComponent<Rigidbody>().linearVelocity.magnitude > 2f)
                {
                    ToggleReverseLight = false;
                }
            }
