...
public class ESMeshDeformer : MonoBehaviour
{
        ...
        int damagedverts, float sqrmaxdist, Vector3[] OriginalMesh)
    {
        ...
        {
            Vector3 damage = (localcollisionforce * ((deformsettings.damageintensity * 2.0f) - Mathf.Sqrt(dist)) / (deformsettings.damageintensity * 2.0f)) * (deformsettings.offsetforce * (1 + myrigidbody.linearVelocity.magnitude * 0.2f));
            vertices[i] += damage;
